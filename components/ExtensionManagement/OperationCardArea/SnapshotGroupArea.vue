    ...
    <ShowMore v-model="showMore">
      <div class="show-more-content">
        ...
        <div class="snapshot-item" v-for="item in snapshotsStore.snapshots" :key="item.id">
          <div class="snapshot-item-main" @click="toggleActive(item.name)" @dblclick="snapshotsStore.tryDeleteSnapshot(item.name)">
            <div class="snapshot-item-name" :class="item.name === activeSnapshotName ? 'active' : ''">{{ item.name }}</div>
            <div class="snapshot-item-time">{{ item.time }}</div>
          </div>
          <div class="snapshot-item-actions">
            <button @click="snapshotsStore.restoreSnapshot(item.name)">恢复</button>
          </div>
        </div>
      </div>
    </ShowMore>
