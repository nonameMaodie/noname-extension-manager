...
import { classes, extensionList } from "./libExtensions.js";
...
export const useExtensionsStore = defineStore("extensions", () => {
  ...
  function showExtensionDetail(ext) {
    ...
    const mask = ui.create.div(
      document.querySelector("#window>.kzgj-extension-manager-app")
    );
    mask.style.cssText = `
			position: absolute;
			top: 0;
    ...
    const element = ui.menuContainer.querySelector(
      ".menu-content .menubutton.active"
    );
    if (!element) return;
    ...
    if (result) {
      try {
        ...
        const div = ui.create.div(
          "",
          "<span>确认删除</span>",
          ui.create.div(ui.create.div())
        );
        lib.extensionMenu[`extension_${ext.name}`].delete.onclick.call(div);
      } catch {
        if (lib.config.extensions.includes(ext.name))
          game.removeExtension(ext.name);
      } finally {
        extensionList.value = extensionList.value.filter(
          (e) => e.name !== ext.name
        );
      }
    }
