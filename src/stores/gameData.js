...
export const useGameDataStore = defineStore("gameData", () => {
  const folderPath =
    getDevice() !== "mobile"
      ? `${basic.extensionDirectoryPath}/gameData`
      : `${basic.extensionDirectoryPath}/gameDataForMobile`;
  const toast = useToast();
  const gameDataList = ref(
    Array.from({ length: 16 }, (_, i) => createInitialDataInfo(i + 1))
  );
  ...
  async function createData({ id, name }) {
    let data;
    const get_data = (data) => lib.init.encode(JSON.stringify(data));
    if (lib.db) {
