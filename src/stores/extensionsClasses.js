...
import { classes, extensionList, recordClasses } from "./libExtensions.js";

export const useExtensionsClassesStore = defineStore(
  "extensionsClasses",
  () => {
    ...
    const unaddedExtList = computed(() => {
      return extensionList.value.filter(
        (ext) => !currentClass.value.extensions.includes(ext)
      );
    });
    // 添加新的扩展分类
    ...
    function renameExtensionClass(extClass, newName) {
      ...
      if (
        classes.value.some((item) => item != extClass && item.name === newName)
      ) {
        toast.error("已存在同名分类!");
        return;
      }
