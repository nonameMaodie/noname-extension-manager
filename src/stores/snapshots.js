...
export const useSnapshotsStore = defineStore("snapshot", () => {
  ...
  async function readSnapshot(fileName) {
    const info = await lib.init.promises.json(
      `${basic.extensionDirectoryPath}/snapshots/${fileName}`
    );
    return info;
  }
  ...
  async function readAllSnapshots() {
    const [_, files] = await game.promises.getFileList(
      `${basic.extensionDirectoryPath}/snapshots`
    );
    snapshots.value = [];
  ...
  async function deleteSnapshot(fileName) {
    await game.promises.removeFile(
      `${basic.extensionDirectoryPath}/snapshots/${fileName}`
    );
  }
  ...
  function createSnapshot(name) {
    return {
      ...
      data: Object.assign(
        {},
        ...extensionList.value.map((ext) => ({ [ext.name]: ext.enabled }))
      ),
